...
Note: Calls to scipy's lstsq function are replaced by the simpler
_lstsq_univariate function defined within, which is faster than lstsq for
regressions for one endogenous variable.
"""

import numpy as np
import lstsq_univariate

_lstsq_univariate = lstsq_univariate.lstsq_univariate


def lowess_new(endog, exog, frac = 2./3, it = 3):
    """
...
def _lowess_initial_fit(x_copy, y_copy, k, n):
   ...
   """
    ...
    for i in xrange(n):
        ...
        beta = _lstsq_univariate(weights[i,:].reshape(k,1) * X, y_i)
...
def _lowess_robustify_fit(x_copy, y_copy, fitted, weights, k, n):
    ...
    for i in xrange(n):
        ...
        beta = _lstsq_univariate(total_weights * X, y_i)
